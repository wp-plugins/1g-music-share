...
//原window.php
?>
<script type="text/javascript">function showFav(user){str=user;xmlHttp=GetXmlHttpObject();if(xmlHttp==null){alert("Browser does not support HTTP Request");return;}var url="<? echo $wp1g_plugin_url; ?>fav.php";url=url+"?user="+encodeURIComponent(str);xmlHttp.onreadystatechange=stateChangedFav;xmlHttp.open("GET",url,true);xmlHttp.send(null);document.getElementById("searchlistsfav").innerHTML='<p>获取收藏记录中...<p>';}function showFav_input(){str=document.getElementById("wplay_user").value;xmlHttp=GetXmlHttpObject();if(xmlHttp==null){alert("Browser does not support HTTP Request");return;}var url="<? echo $wp1g_plugin_url; ?>fav.php";url=url+"?user="+str;xmlHttp.onreadystatechange=stateChangedFav;xmlHttp.open("GET",url,true);xmlHttp.send(null);document.getElementById("searchlistsfav").innerHTML='<p>获取收藏记录中...<p>';}function stateChangedFav(){if(xmlHttp.readyState==4||xmlHttp.readyState=="complete"){if(xmlHttp.status==200){document.getElementById("searchlistsfav").innerHTML=xmlHttp.responseText;}}}function enterIn_fav(evt){var evt=evt?evt:(window.event?window.event:null);if(evt.keyCode==13){showFav_input();}}function insert1gByFavID(id){var tagtext;var play=id;var autoplay=document.getElementById("autoplay_fav").checked;tagtext='[music1g play=#'+play+' autoplay='+autoplay+']';var win=window.dialogArguments||opener||parent||top;win.send_to_editor(tagtext);return false;}function GetXmlHttpObject(){var xmlHttp=null;try{xmlHttp=new XMLHttpRequest()}catch(e){try{xmlHttp=new ActiveXObject("Msxml2.XMLHTTP")}catch(e){xmlHttp=new ActiveXObject("Microsoft.XMLHTTP")}}return xmlHttp}</script>
<style type="text/css">body{font-size:13px;font-family:"微软雅黑","文泉驿微米黑","Comic Sans MS"}
#listbox{font-size:12px;font-family:"微软雅黑","文泉驿微米黑","Comic Sans MS"}
#sharebutton{width:30px;color:#099}</style>
<table><tbody><tr><td width="400px"><input type="checkbox" id="autoplay_fav" /> 自动播放</td></tr><tr><td width="400px"><p style="font-size: 13px;" align="center">请输入亦歌用户名</p></td></tr><tr><td align="center"><input type="text" onkeydown="enterIn_fav(event);" id="wplay_user" size="40px" style="font-size: 13px;" value="<?php $options = get_option('wp1gsettings');echo $options['user']; ?>" /><input type="button" id="insert2" value="读取" onClick="showFav_input();" style="font-size: 13px;" /></td></tr></tbody></table><div id="searchlistsfav"></div>
<?php
$options = get_option('wp1gsettings');
